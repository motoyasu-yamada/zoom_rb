  ...
  module Actions
    module Token
      def access_tokens(*args)
        ...
        options.require(%i[client_id　auth_code redirect_uri])
        response = self.class.post("/oauth/token?grant_type=authorization_code&client_id=#{Zoom.configuration.client_id}&code=#{options[:auth_code]}&redirect_uri=#{options[:redirect_uri]}", headers: oauth_request_headers, base_uri: 'https://zoom.us/')
